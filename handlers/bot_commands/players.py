...
from handlers.database import get_users_list,clear_user_list, get_user_stats
...
async def get_users(message: types.Message):
    ...
    await message.answer(list_of_users)
...
async def get_users(message: types.Message):
    ...
    create_task(remove_mes(message,5))
...
async def clear_users(message: types.Message):
    await clear_user_list()
    await message.answer("Список очищен, сладенький🤭")